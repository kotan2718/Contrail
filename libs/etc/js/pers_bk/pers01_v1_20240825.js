...
function range_to_html() {
    document.getElementById('m_xMin').value = m_xMin.toFixed(1);
    document.getElementById('m_xMax').value = m_xMax.toFixed(1);
    document.getElementById('m_yMin').value = m_yMin.toFixed(1);
    document.getElementById('m_yMax').value = m_yMax.toFixed(1);
    document.getElementById('m_zMin').value = m_zMin.toFixed(1);
    document.getElementById('m_zMax').value = m_zMax.toFixed(1);
}
...
function changeType() {
    ...
    switch (typeAry[0]) {
        ...
        case "03":
            switch (typeAry[1]) {
                case "01":  // f(x,y) = x^3 + y^3 //20230815 add
                    m_index = 301;
                    if (resetFlg) {
                        return;
                    }
                    ...
                    break;
                case "02":  // f(x,y) = cos(y) - cos(x) //20230815 add
                    m_index = 302;
                    if (resetFlg) {
                        return;
                    }
                    ...
                    break;
                case "03":  // f(x,y) = (x^2 + y^2 + a^2)^2 - 4a^2x^2 - b^4, Cassini Ovals, a = 0.6, b=0.4 //20230815 add
                    m_index = 303;
                    if (resetFlg) {
                        return;
                    }
                    ...
                    changeMesh();
                    // 係数デフォルト値
                    document.getElementById('ma').value = 1.0;
...
//                    document.getElementById('fxy').innerText = "$ f(x, y) = (x^2 + y^2 + a^2)^2 - 4a^2x^2 - b^4 $";
                    document.getElementById('fxy').innerText = "$ f(x, y) = (x^2 + y^2 + a^2)^2 - 4a^2x^2 - 4*b $";
                    // 係数入力ボックスを表示する
                    dispCoef(1);
                    // 段落のマージンを設定する
                    changeDivMargin(true);
                    break;
                case "04":  // f(x,y) = x^a - 2x^b*y + y^a //20230815 add
                    m_index = 304;
                    if (resetFlg) {
                        return;
                    }
...
//            FXY = math.pow(x*x + y*y + ma*ma, 2) - 4.0 * ma*ma * x*x - mb*mb*mb*mb;
            FXY = math.pow(x*x + y*y + ma*ma, 2) - 4.0 * ma*ma * x*x - 4*mb;
            return FXY;
        case 304:
